...
export const GeographicTableContainer = (props: TableHeaderProps) => {
  ...
  return (
    ...
    >
      <div>
        <h2 className="text-[11px] mb-5 underline font-bold tracking-[0.15em] text-muted-foreground/90 uppercase leading-none  whitespace-nowrap">
          {placeHierarchy}
        </h2>
        {/*nav link header*/}
        {crumbs.length > 0 && (
          <nav className="flex items-center gap-1 text-xs uppercase tracking-[0.2em] font-bold text-muted-foreground/60 mb-2">
              ...
              {crumbs.map((crumb, index) => (
                <React.Fragment key={index}>
                  ...
                  {index < crumbs.length - 1 && (
                    <span className="px-1 text-muted-foreground/20">{"<"}</span>
                  )}
                </React.Fragment>
              ))}
            </div>
